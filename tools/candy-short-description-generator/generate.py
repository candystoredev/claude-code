#!/usr/bin/env python3
"""
Candy short product description generator using Claude API with image analysis.
...
def load_completed_skus(filepath: str) -> set[str]:
    ...
    with open(filepath, newline="", encoding="utf-8-sig") as f:
        ...
        for row in reader:
            ...
            if sku and row.get("new_short_description", "").strip():
                completed.add(sku)
    return completed
...
def main():
    parser = argparse.ArgumentParser(description="Generate candy short product descriptions with Claude API")
    parser.add_argument("--input", default=None, help="Input CSV path (default: input/products.csv)")
    parser.add_argument("--output", default=None, help="Output CSV path (default: output/products_with_short_descriptions.csv)")
    ...
    if "new_short_description" not in fieldnames:
        fieldnames.append("new_short_description")
    ...
    for i, row in enumerate(rows):
        ...
        try:
            ...
            row["new_short_description"] = description
            row["generation_status"] = "success"
            processed += 1
            print("OK")
        except anthropic.APIError as e:
            row["new_short_description"] = ""
